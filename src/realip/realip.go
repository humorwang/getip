...
func FromRequest(r *http.Request) string {
	// Fetch header value
	headerFiled := []string{"X-Forwarded-For", "X-Original-Forwarded-For"}
	xRealIP := r.Header.Get("X-Real-Ip")
	xForwardedFor := r.Header.Get("X-Forwarded-For")
	XOriginalForwardedFor := r.Header.Get("X-Original-Forwarded-For")
	// If both empty, return IP from remote address
	if xRealIP == "" && xForwardedFor == "" && XOriginalForwardedFor == "" {
		var remoteIP string
	...
	for _, header := range headerFiled {
		for _, address := range strings.Split(r.Header.Get(header), ",") {
